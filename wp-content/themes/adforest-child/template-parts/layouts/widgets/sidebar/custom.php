...
if (isset($_GET['cat_id']) && $_GET['cat_id'] != "" && is_numeric($_GET['cat_id'])) { // for search page
        ...
        if($ad_cat_sub_sub != ""){
             $term_id = $ad_cat_sub_sub;
        }
        if($ad_cat_sub_sub_sub != ""){
             $term_id = $ad_cat_sub_sub_sub;
        }
          if($ad_cat_sub_sub_sub_sub != ""){
             $term_id = $ad_cat_sub_sub_sub_sub;
        }
...
if ($custom_cat_flag && $term_id != '') {
  ...
  <div class="panel-heading" role="tab" id="heading-' . $rand_ids . '">
     <h4 class="ad-widget-title"><a class="'.esc_attr($toggle).'" role="button" data-bs-toggle="collapse" data-parent="#accordion" href="#custom-cate-' . $rand_ids . '" aria-expanded="' . $open_widget_cus . '" aria-controls="heading-' . $rand_ids . '"><i class="more-less fa fa-plus"></i>' . esc_html($title = empty($instance['title']) ? '' : apply_filters('widget_title', $instance['title'])) . ' ' . esc_html($r['titles']) . '</a></h4></div>
